...
public class TeleportButtonInitialize : IVRCSDKBuildRequestedCallback
{
    private static bool SetSerializedProperties()
    {
        ...
        if (!GetSerializedObjects<TeleportButton>(out SerializedObject[] serializedObject)) return false;
        foreach (var obj in serializedObject)
        {
            PopulateSerializedProperty<Teleporter>(obj, nameof(TeleportButton.teleporter));
        }
